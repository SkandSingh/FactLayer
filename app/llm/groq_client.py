"""Groq implementation of the provider-agnostic `LLMClient` interface.

Groq (groq.com) hosts open-weight models (Llama, GPT-OSS, Qwen, etc.) behind
a fast inference API that's OpenAI-Chat-Completions-compatible, so this
client talks to it directly over HTTP via `httpx` rather than depending on
a provider-specific SDK. Not to be confused with xAI's "Grok" model family
-- a different company, a different API, and (notably) a different API key
prefix (Groq keys start with `gsk_`).
"""
import asyncio
import logging
...
class GroqClient(LLMClient):
    """LLMClient backed by Groq's API (OpenAI-compatible chat endpoint)."""
    ...
    async def _call_once(self, prompt: str) -> str:
        """Make a single timed call to the underlying Groq API.
    ...
    async def generate(self, prompt: str) -> str:
        """Send prompt to Groq, return the raw text response.
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if is_last_attempt:
                    logger.warning(
                        "Groq call failed on attempt %d/%d (final attempt, "
                        "giving up): %s",
                        attempt + 1,
                        max_attempts,
                        exc,
                ...
                logger.warning(
                    "Groq call failed on attempt %d/%d: %s; retrying in %ds",
                    attempt + 1,
                    max_attempts,
                    exc,
                    delay,
